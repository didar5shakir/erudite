"""
Шаг 7b: Помесячные pageviews для 17,609 Wiki-фигур.
Версия с asyncio + aiohttp (10 параллельных запросов).

Фаза 1 — Wikidata sitelinks (sync, ~1 мин, обычно уже в кэше).
Фаза 2 — Wikimedia Pageviews API (async, ~1-2 ч вместо 11).

Кэш: data/processed/wiki_pv_cache.json — перезапуск продолжает.
Финал: data/processed/wiki_pv_monthly.csv
...
IN_PATH    = ROOT / "data" / "processed" / "wiki_top_humans_aggregated.csv"
CACHE_PATH = ROOT / "data" / "processed" / "wiki_pv_cache.json"
OUT_PATH   = ROOT / "data" / "processed" / "wiki_pv_monthly.csv"
...
def main():
    ...
    wd_ids = df["wikidata_id"].astype(str).tolist()
    print(f"Wiki-фигур: {len(wd_ids):,}")
